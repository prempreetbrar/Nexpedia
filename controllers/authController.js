...
exports.protect = errorHandling.catchAsync(async (request, response, next) => {
...
exports.loginUser = errorHandling.catchAsync(async (request, response) => {
  ...
  if (!user || !(await user.isPasswordCorrect(password, user.password)))
    throw new errorHandling.AppError("Incorrect email or password", 401);

  createSendToken(user, 200, request, response);
});

exports.logoutUser = errorHandling.catchAsync(async (request, response) => {
